...
public:
	...
	void mainMenu() {
		...
		mainMenu->addItem("Выполнить вариант 09"); //4
		...
		while (resultSelectedItem != exitInt) {
			...
			case 4: //Выполнить вариант XX
				...
				//printAllSurName_Name_MName_bYaear_AvrMarks();
				sort();
				printAllSurName_Name_MName_bYaear_Mark5();
				_getch();
	...
	//sort
	void processingAvrMark5() {
		struct StudentNode* current = myHead;
		while (current) {
			current->avrMark = getMarks5(current);
			current = current->next;
		}
	}
	...
	void printAllSurName_Name_MName_bYaear_Mark5() {
		struct StudentNode* current = myHead;
		cout << endl;
		cout << "ФИО любимого ученика -> год рождения -> количество пятерок за второй семак" << endl;
		cout << endl;
		while (current) {
			//cout << " " << current->data;
